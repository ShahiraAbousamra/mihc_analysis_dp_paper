...
if __name__ == '__main__':
    ...
    ######## unet #########
    conc_dir_name = 'unet_retrain_e410_19patches'

    thresh_dict = {'cd3':0.0, 'cd4':0.0, 'cd8':0.0, 'cd16':0.0, 'cd20':0.0, 'k17':0.0, 'hematoxilin':0.0, 'background':0.0}

    ######## Processing #########
    in_dir = os.path.join(root, conc_dir_name)
    out_dir = os.path.join(root, conc_dir_name + '-a_conc')
    if(not os.path.isdir(out_dir)):
        os.mkdir(out_dir)
    files = glob.glob(in_dir+'/*.npy') 
    for file in files:
        print(file)
        create_seg_map(file, out_dir, thresh_dict, size_thresh_dict, resize_ratio, do_thresh_size=False, do_fill_holes=False, do_mz_map=False, do_dilate=False) # sup
        sys.stdout.flush()
