...
def main(args):
    ...
    if(cost_func_class_name == 'CrossEntropyCostSeg'):
        from ..sa_cost_func.ce_cost_seg import CrossEntropyCostSeg;
        cost_func = CrossEntropyCostSeg(n_classes = n_classes, deviceID=device, kwargs=cost_params);
    else:
        print('error: cost function class name \'{}\' is not supported by runner'.format(cost_func_class_name));
        sys.exit();        

    # Configure Network architecture
    if(network_class_name == 'MultiplexSegUnet'):
        from ..sa_networks.multiplex_seg_unet import MultiplexSegUnet;
        cnn_arch = MultiplexSegUnet(n_channels = n_channels, n_classes = n_classes, model_out_path = model_path, model_base_filename = model_base_filename, model_restore_filename = model_restore_filename, cost_func = cost_func \
    ...
    if(is_test == False):
        if(train_dataprovider_class_name == 'MultiplexSegDataProviderFromNumpyDots'):
            from ..sa_data_providers.multiplex_seg_data_provider_from_npy_dots import MultiplexSegDataProviderFromNumpyDots;
            train_data_provider = MultiplexSegDataProviderFromNumpyDots( \
                is_test=is_test \
        ...
        if(has_validation):
            if(validate_dataprovider_class_name == 'MultiplexSegDataProviderFromNumpyDots'):
                from ..sa_data_providers.multiplex_seg_data_provider_from_npy_dots import MultiplexSegDataProviderFromNumpyDots;
                validate_data_provider = MultiplexSegDataProviderFromNumpyDots( \
                    is_test=is_test \
        ...
        elif(test_dataprovider_class_name == 'MultiplexSegDataProviderFromNumpyDotsTest'):
            from ..sa_data_providers.multiplex_seg_data_provider_from_npy_dots_test import MultiplexSegDataProviderFromNumpyDotsTest;
            test_data_provider = MultiplexSegDataProviderFromNumpyDotsTest( \
                is_test=is_test \
                , filepath_data = test_filepath_data \
                , filepath_label = None \
                , n_channels = n_channels \
    ...
    if(is_test == False):
        ...
        if(trainer_class_name == 'MultiplexSegTrainer'):
            from ..sa_trainers.sa_net_train_multiplex_seg import MultiplexSegTrainer;
            trainer = MultiplexSegTrainer(cnn_arch \
                , cnn_arch_module \
                , train_data_provider \
        ...
        if(tester_class_name == 'MultiplexSegTester'):
            from ..sa_testers.sa_net_test_mypytorch_multiplex_seg import MultiplexSegTester;
            tester = MultiplexSegTester(cnn_arch \
                , cnn_arch_module \
                , test_data_provider \
                , session_config=session_config \
                , device = device \
