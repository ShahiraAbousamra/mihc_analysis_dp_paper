    ...
    def test(self, do_init, do_restore, do_load_data):
        ...
        with torch.no_grad():
            ...
            while(batch_x is not None):
                ...
                batch_y = self.cnn_arch(batch_x);

                batch_count = batch_x.shape[0];
                total_count += batch_count ;

                batch_y = batch_y.detach().cpu().numpy().astype(np.float16);
                for i in range(batch_y.shape[0]):
                    batch_y[i].dump(os.path.join(self.output_dir, os.path.splitext(os.path.split(batch_filenames[i])[1])[0] + '.npy'));
