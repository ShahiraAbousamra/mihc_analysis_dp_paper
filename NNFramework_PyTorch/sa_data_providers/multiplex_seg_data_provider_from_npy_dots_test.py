...
class MultiplexSegDataProviderFromNumpyDotsTest(AbstractDataProvider):
    ...
    def load_data(self):
        ...
        img_path_files = glob.glob(os.path.join(self.filepath_data, "*.png"));
    ...
    def get_next_n(self, n:int):
        ...
        #if(self.pad_y > 0 or self.pad_x > 0):
        #    print('before resize data_points.shape=', data_points.shape)
        #    data_points = cv.resize(data_points,(data_points.shape[0], data_points.shape[1], data_points.shape[-1]+self.pad_x,data_points.shape[-2]+self.pad_y))
        #    print('after resize data_points.shape=', data_points.shape)
    ...
    def augment(self, data_point,label):
        ...
        if(op == 1):
            ...
            label = np.rot90(label, k=2, axes=(0,1)).copy();
        elif(op == 5):
            data_point2 = np.rot90(data_point, k=1, axes=(0,1)).copy();
            label = np.rot90(label, k=1, axes=(0,1)).copy();
        elif(op == 6):
            data_point2 = np.rot90(data_point, k=3, axes=(0,1)).copy();
            label = np.rot90(label, k=3, axes=(0,1)).copy();
        #if(self.aug_rot_random):
    ...
    def load_datapoint(self, filepath):
        if(filepath[-4:] == '.npy'):
            ...
            label = img_data[:,:,3];
            # handle background
            has_label = img_data[:,:,4:].sum(axis=-1)
            label[np.where(has_label == 0)] = 7
        else:
            img = io.imread(filepath);
            if(img.shape[2] > 3): # remove the alpha
                img = img[:,:,0:3];
